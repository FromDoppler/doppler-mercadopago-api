...
namespace Doppler.MercadoPagoApi.Validators
{
    ...
    {   
        public CardDtoValidator()
        {
            ...
            RuleFor(x => x.ExpirationYear).NotEmpty().WithMessage("ExpirationYear cannot be empty");
            RuleFor(x => x.ExpirationMonth).NotEmpty().WithMessage("ExpirationMonth cannot be empty");
        }
    }
}
